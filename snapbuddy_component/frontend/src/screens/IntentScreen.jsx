...
export default function IntentScreen({ setScreen, vibeText, setVibeText, refImages, setRefImages, fileInputRef, occasion, setOccasion, generateTags, nav, isAnalyzing, analyzeError, setAnalyzeError }) {
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", display: "flex", flexDirection: "column" }}>
      ...
      <div className="px-5 py-6 space-y-6 flex-1">
        <div><label className="text-xs uppercase tracking-wider text-stone-500 block mb-2">อธิบาย vibe</label><textarea value={vibeText} onChange={(e) => setVibeText(e.target.value)} className="w-full rounded-3xl p-4 text-sm outline-none" style={{ background: "rgba(240,230,216,0.6)", minHeight: "112px" }} placeholder="เช่น old money โทนครีม แสงเช้า ดูแพงแต่สบายๆ" /></div>
        ...
        {analyzeError ? <div className="rounded-2xl px-4 py-3 text-sm text-amber-900" style={{ background: "#fef3c7" }}>{analyzeError}</div> : null}
      </div>
      <div className="p-4 border-t border-stone-200" style={{ background: "#faf6f1" }}>
        <button disabled={(!vibeText.trim() && !refImages.length) || isAnalyzing} onClick={handleAnalyze} className={btnPrimary} style={vibeText.trim() || refImages.length ? btnStyle : { background: "#d6d3d1" }}>
          {isAnalyzing ? "Analyzing..." : "Analyze your vibe ✨"}
        </button>
      </div>
    </div>
  );
